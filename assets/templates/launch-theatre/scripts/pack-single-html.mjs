...
const output = path.resolve(root, args.get("output") || "dist/launch-theatre-keynote-deck.single.html");
...
function inlineUrlRefs(css, fromFile) {
  return css.replace(/url\((['"]?)([^)'"]+)\1\)/g, (whole, quote, rawUrl) => {
    ...
    return `url("data:${mimeFor(assetPath)};base64,${data.toString("base64")}")`;
  });
}
...
function inlineScripts(html, htmlFile) {
  return html.replace(/<script\s+([^>]*?)src=(["'])([^"']+)\2([^>]*)><\/script>/gi, (whole, before, quote, src) => {
    if (/^https?:/i.test(src)) return whole;
    const jsFile = path.resolve(path.dirname(htmlFile), src);
    const js = readText(jsFile).replace(/<\/script/gi, "<\\/script");
